...
impl<'a> PlacementResolver<'a> {
    ...
    /// Resolve an `IfcLocalPlacement` step_id to a world matrix.
    pub fn world(&mut self, placement_id: u64) -> Mat4 {
    ...
    fn resolve(&mut self, placement_id: u64) -> Mat4 {
        ...
        let local_axis = match parse_field(fields[1]) {
            Field::Ref(id) => axis_placement_3d_from_id(self.table, id),
            _ => Mat4::IDENTITY,
        };
