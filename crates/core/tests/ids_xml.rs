...
fn ids_minimal_document_to_expected_ir() {
    let x = doc(
        r#"<specification name="Walls" ifcVersion="IFC2X3 IFC4X3_ADD2" identifier="W-1" description="d" instructions="i">
...
"#,
    ...
    match &s.applicability[1] {
        Facet::PartOf { relation, .. } => {
            assert_eq!(*relation, Some(Relation::VoidsElementFillsElement))
        }
        f => panic!("{f:?}"),
    }
    ...
    let kinds: Vec<(&str, FacetCardinality)> = s
        .requirements
        .iter()
        .map(|r| (r.facet.kind(), r.cardinality))
        .collect();
    assert_eq!(
        kinds,
        vec![
    ...
    assert_eq!(
        j,
        parse(&x).map(|d| d.to_canonical_json()).unwrap_or_default()
    );
}
...
fn ids_spec_cardinality_combinations() {
    ...
    assert_eq!(
        card(r#"minOccurs="0" maxOccurs="unbounded""#).ok(),
        Some(SpecCardinality::Optional)
    );
    assert_eq!(
        card(r#"minOccurs="0" maxOccurs="0""#).ok(),
        Some(SpecCardinality::Prohibited)
    );
    ...
    assert_eq!(
        card(r#"maxOccurs="unbounded""#).ok(),
        Some(SpecCardinality::Required)
    );
    assert_eq!(
        card(r#"minOccurs="0""#).ok(),
        Some(SpecCardinality::Optional)
    );
    ...
    for bad in [
        ...
        assert_invalid(
            &spec(bad, WALL, ""),
            "is not an IDS 1.0 specification cardinality",
        );
    }
...
fn ids_rejects_prohibited_spec_with_requirements() {
    ...
    assert_invalid(
        &spec(r#"minOccurs="0" maxOccurs="0""#, WALL, attr),
...
fn ids_rejects_structure_violations() {
    let ok_spec = r#"<specification name="S" ifcVersion="IFC4"><applicability>"#.to_string()
        + WALL
        + "</applicability></specification>";
    // unknown element
    assert_invalid(
        &doc(&ok_spec.replace("</applicability>", "<foo/></applicability>")),
        "unexpected element <foo>",
    );
    // unknown attribute
    assert_invalid(
        &doc(&ok_spec.replace("<applicability>", r#"<applicability color="red">"#)),
        "unexpected attribute 'color'",
    );
    assert_invalid(
        &doc(&ok_spec.replace("<entity>", r#"<entity instructions="x">"#)),
        "unexpected attribute 'instructions'",
    );
    assert_invalid(
        &doc(&ok_spec.replace("<entity>", r#"<entity cardinality="optional">"#)),
        "unexpected attribute 'cardinality'",
    );
    assert_invalid(
        &doc(&ok_spec.replace("<applicability>", r#"<applicability xsi:type="x">"#)),
        "xsi:type",
    );
    // missing required element / attribute
    assert_invalid(&doc(""), "missing required element <specification>");
    assert_invalid(
        &doc(r#"<specification name="S" ifcVersion="IFC4"></specification>"#),
        "missing required element <applicability>",
    );
    assert_invalid(&doc(&ok_spec.replace(r#" name="S""#, "")), "'name'");
    assert_invalid(
        &doc(&ok_spec.replace(r#" ifcVersion="IFC4""#, "")),
        "'ifcVersion'",
    );
    assert_invalid(
        &(HEAD.replace("<info><title>T</title></info>", "<info></info>") + TAIL),
        "missing required element <title>",
    );
    assert_invalid(
        &(HEAD.replace("<title>T</title>", "") + TAIL),
        "missing required element <title>",
    );
    ...
    assert_invalid(
        &spec(
            REQ,
            WALL,
            "<classification><value><simpleValue>X</simpleValue></value></classification>",
        ),
        "missing required element <system>",
    );
    // idsValue with both / neither child
    assert_invalid(
        &spec(REQ, "<entity><name></name></entity>", ""),
        "exactly one",
    );
    assert_invalid(
    ...
    assert_invalid(
        &spec(REQ, &format!("hello{WALL}"), ""),
        "unexpected text 'hello'",
    );
    // element inside simpleValue
    assert_invalid(
        &spec(
            REQ,
            "<entity><name><simpleValue><b/>IFCWALL</simpleValue></name></entity>",
            "",
        ),
        "text only",
    );
...
fn ids_rejects_bad_tokens() {
    let v = |ver: &str| {
        doc(&format!(
            r#"<specification name="S" ifcVersion="{ver}"><applicability>{WALL}</applicability></specification>"#
        ))
    };
    ...
    let part = |attr: &str| {
        spec(REQ, WALL, &format!("<partOf {attr}><entity><name><simpleValue>IFCBUILDING</simpleValue></name></entity></partOf>"))
    };
    assert_invalid(
        &part(r#"relation="IFCRELVOIDSELEMENT""#),
        "relation 'IFCRELVOIDSELEMENT'",
    );
    assert_invalid(&part(r#"relation="ifcrelaggregates""#), "relation");
    assert_invalid(
        &part(r#"cardinality="optional""#),
        "cardinality 'optional' is not allowed on <partOf>",
    );
    ...
    let attr_card = |c: &str| {
        spec(
            ...
            WALL,
            &format!(
                r#"<attribute cardinality="{c}"><name><simpleValue>Name</simpleValue></name></attribute>"#
            ),
        )
    };
    ...
    let dt = |d: &str| {
        spec(
            ...
            WALL,
            &format!(
                r#"<property dataType="{d}"><propertySet><simpleValue>P</simpleValue></propertySet><baseName><simpleValue>B</simpleValue></baseName></property>"#
            ),
        )
    };
    ...
    let info = |inner: &str| {
        HEAD.replace(
            "<info><title>T</title></info>",
            &format!("<info><title>T</title>{inner}</info>"),
        ) + &spec_body()
            + TAIL
    };
    ...
    // info out of order
    assert_invalid(
        &info("<date>2026-09-24</date><author>a@b.no</author>"),
        "expected, in this order",
    );
}
...
fn spec_body() -> String {
    format!(
        r#"<specification name="S" ifcVersion="IFC4"><applicability>{WALL}</applicability></specification>"#
    )
}
...
fn ids_restriction_parsing_and_rejections() {
    ...
    match &d.specs[0].requirements[0].facet {
        Facet::Attribute {
            ...
            assert_eq!(
                (r.min_length, r.max_length, r.length),
                (Some(1), Some(2), None)
            );
        }
        f => panic!("{f:?}"),
    ...
    let other_prefix = restr(
        r#"<q:restriction xmlns:q="http://www.w3.org/2001/XMLSchema" base="q:integer"><q:minInclusive value="3"/></q:restriction>"#,
    );
    assert!(parse(&other_prefix).is_ok());

    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"/>"#),
        "has no facets",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:decimal"><xs:minInclusive value="1"/></xs:restriction>"#,
        ),
        "not an IDS 1.0 restriction base",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="foo:string"><xs:length value="1"/></xs:restriction>"#),
        "@base",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:string"><xs:length value="1"/><xs:length value="2"/></xs:restriction>"#,
        ),
        "at most once",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:double"><xs:minInclusive value="1"/><xs:minInclusive value="2"/></xs:restriction>"#,
        ),
        "at most once",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"><xs:length value="-1"/></xs:restriction>"#),
        "non-negative integer",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"><xs:length/></xs:restriction>"#),
        "'value'",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:string"><xs:length value="1" fixed="true"/></xs:restriction>"#,
        ),
        "'fixed'",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"><xs:foo value="1"/></xs:restriction>"#),
        "not an xs:restriction facet",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:double"><xs:minInclusive value="1,5"/></xs:restriction>"#,
        ),
        "not a valid xs:double literal",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:integer"><xs:enumeration value="42.0"/></xs:restriction>"#,
        ),
        "not a valid xs:integer literal",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"><xs:minInclusive value="1"/></xs:restriction>"#),
        "not allowed on base xs:string",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:double"><xs:length value="1"/></xs:restriction>"#),
        "not allowed on base xs:double",
    );
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:boolean"><xs:enumeration value="true"/></xs:restriction>"#,
        ),
        "not allowed on base xs:boolean",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"><xs:pattern value="a*?"/></xs:restriction>"#),
        "quantifier",
    );
    assert_invalid(
        &restr(r#"<xs:restriction base="xs:string"><xs:pattern value="(a)\1"/></xs:restriction>"#),
        "not an XSD escape",
    );
    ...
    assert!(parse(&restr(
        r#"<xs:restriction base="xs:string"><xs:pattern value="\p{IsThai}+"/></xs:restriction>"#
    ))
    .is_ok());
    // Bounds on temporal bases parse (evaluation is Unsupported for now).
    assert!(parse(&restr(
        r#"<xs:restriction base="xs:date"><xs:minInclusive value="2020-01-01"/></xs:restriction>"#
    ))
    .is_ok());
    assert_invalid(
        &restr(
            r#"<xs:restriction base="xs:date"><xs:minInclusive value="01.01.2020"/></xs:restriction>"#,
        ),
        "xs:date",
    );
}
...
fn ids_audit_rules_via_parse() {
    let prop = |dt: &str, v: &str| {
        spec(
            ...
            WALL,
            &format!(
                r#"<property dataType="{dt}"><propertySet><simpleValue>P</simpleValue></propertySet><baseName><simpleValue>B</simpleValue></baseName><value>{v}</value></property>"#
            ),
        )
    };
    let sv = |s: &str| format!("<simpleValue>{s}</simpleValue>");
    assert_invalid(
        &prop("IFCBOOLEAN", &sv("TRUE")),
        "not a valid IFCBOOLEAN literal",
    );
    ...
    assert_invalid(
        &prop(
            "IFCREAL",
            r#"<xs:restriction base="xs:string"><xs:pattern value=".*"/></xs:restriction>"#,
        ),
    ...
    assert_invalid(
        &req_ent(&sv("IFCSLAB")),
        "can never match the applicability entity 'IFCWALL'",
    );
    assert!(parse(&req_ent(&sv("IFCWALL"))).is_ok());
    assert!(parse(&req_ent(
        r#"<xs:restriction base="xs:string"><xs:pattern value="IFCWALL.*"/></xs:restriction>"#
    ))
    .is_ok());
}
...
fn ids_encodings() {
    ...
    assert!(
        matches!(parse_ids(&bytes), Err(IdsError::Unsupported { ref feature, .. }) if feature == "xml-encoding:ISO-8859-1")
    );
    // Norwegian text round-trips.
    let no = x.replace("<title>T</title>", "<title>Bæreevne på dør</title>");
    assert_eq!(
        parse(&no).map(|d| d.info.title).ok().as_deref(),
        Some("Bæreevne på dør")
    );
}
...
fn ids_error_carries_line_number() {
    let x = spec(
        REQ,
        WALL,
        "<attribute cardinality=\"bogus\"><name><simpleValue>Name</simpleValue></name></attribute>",
    );
...
fn ids_conformance_full_suite_if_present() {
    let root = std::env::var_os("IFCFAST_IDS_TESTCASES")
        .map(PathBuf::from)
        .or_else(|| {
            std::env::var_os("HOME").map(|h| {
                PathBuf::from(h)
                    .join(".cache/ifcfast/ids-testcases/a67047736aa93586d723329fce3aab1b9ac056af")
            })
        });
