...
- **Ramp-up:** `system_prompt()` (the paste-into-your-prompt summary),
  `example_path()` (bundled fixture for offline demos).
- **Open / inspect:** `open_ifc`, `summary`, `schemas`, `preview`,
  `types`, `by_type`, `diff`.
- **Graph:** `parent`, `children`, `ancestors`, `descendants`,
...
from the packaged copy. If the packaged guide is missing the resource
**raises**; it never quietly substitutes the shorter `system_prompt()`
(GH #157).
...
- `materials(path, guid=…, material_name=…)` — filtered material
  assignments.
- `classifications(path, guid=…, system_name=…, identification=…)` —
  filtered classification references (NS 3451, OmniClass, Uniclass).
...
`children` / `descendants` / `products_in`, where `descendants` from a
building guid used to return every product in the model in one
response. Filter on big models instead of paging; a result of exactly
`limit` items means there are probably more. When a `product_card`
sub-table hits the cap, the response's `truncated` field maps that
table to its total row count, so an incomplete dump is always labelled
as such.

The server's in-process model cache is staleness-checked: if the
file's size or mtime changes between tool calls (re-export from the
authoring tool), it is reopened transparently — you never query a
stale model. It is also **bounded at 8 models, LRU by last use**
(GH #162): each entry pins a fully-parsed model, and the on-disk
parquet cache makes a re-open of an evicted one cheap. `list_open()` /
`close(path)` manage it explicitly.
...
```python
print(ifcfast.system_prompt())   # compact summary — every public method
print(ifcfast.agents_guide())    # THIS document, shipped in the wheel
```

`system_prompt()` returns a compact paragraph covering every public
method; stable across releases (additions only, never reorganisations).
`agents_guide()` returns the full text of this file from package data —
it raises `FileNotFoundError` rather than degrading to the shorter
summary if the install is damaged (GH #157).
...
- Identity / structure: `ifc_id`, `guid`, `class` (normalised — "Wall"
  ...
  see Conventions for the IFC4 door/window correction; since cache
  schema v30 IFC2X3 `IfcSpace` / `IfcRoof` are `None` too, their
  trailing enum is `InteriorOrExteriorSpace` / `ShapeType`, GH #159),
  `object_type`,
...
- Semantic payload: `materials`, `psets`, `quantities`,
  ...
  Contract for `transform` + `representations.vertices_le` (GH #167,
  v30): `world = transform × local_vertex` reproduces the baked mesh
  for every `shared_or_direct` row, including facesets and breps whose
  points were authored at site coordinates — the item's far-origin
  rebase rides on `transform`. Pre-v30 bundles placed such reps up to
  ~0.5 m off through that path (the `bbox_*` / `centroid_xyz` columns
  were always right; only the transform × local reconstruction was
  not).
  Since cache schema v30 (GH #165) the same inheritance pass covers
  `materials` (struct field `source`, `"instance"` / `"type"`) and
  `classifications` (struct field `assignment_source` — `source` on
  that struct is `IfcClassification.Source`, the publisher). The
  in-memory layers `m.materials` / `m.classifications` carry the same
  columns. Before v30 a material or classification bound to the
  *type* produced zero rows for its occurrences.
...
- `source_stats` is what revalidates the federation cache. The cache
  key samples size + head/tail 4 MB of each parquet, which cannot see a
  same-size edit in the MIDDLE of a big file; on a cache hit the
  recorded stats are re-checked against the live files and a mismatch
  re-merges rather than serving a stale substrate — the same contract
  as the parse cache (GH #162).
...
# analysis only — return the DataFrame, write nothing to disk
df = ifcfast.clash("model.bundle/", write_parquet=False)

# guid collision policy for the LIST form (see Federation above):
# "warn" (default) / "fail" / "dedup". Vacuous on a one-element list;
# rejected on a bare path, which carries no federation intent.
df = ifcfast.clash(["ark.bundle/", "rib.bundle/"], on_collision="fail")
```

Full signature: `clash(bundle_dir, *, tolerance_m=0.0,
write_parquet=True, include_classes=None, exclude_self_class=None,
reference_only=(), on_collision="warn")`. `tolerance_m` must be finite and
`>= 0` — a negative or NaN value raises `ValueError` (GH #161; before,
it silently shrank the broad phase and dropped real clashes). The
narrow phase rebases every pair to a scene-wide f64 anchor before f32
packing (GH #156), so clearance verdicts hold at UTM site coordinates.

Run metadata rides on `df.attrs`: `geometryless_skipped`,
`narrow_phase_residuals` (count), `narrow_phase_residual_details`
(GH #161 — one dict per candidate pair whose mesh could not be built:
`ifc_id_a/b`, `guid_a/b`, `class_a/b`, `source_model_a/b`, `side`
(`"a"` / `"b"` / `"both"`), `reason`; empty when every pair built —
this is the list to inspect when Solibri flags a pair ifcfast does
not), `pair_count`, `tolerance_m`, `clash_ms`,
and — only when `write_parquet=True` — `clashes_parquet` (absolute
path of the written file). A federated run adds `federated_dir` (the
content-keyed cache dir holding the merged substrate, where
`clashes.parquet` lands) and `federation` (the `federate()` sidecar).
...
a DATA section that stops early on a stray byte such as a doubled `;;`
(GH #148 — before, every record after it silently vanished and QTO /
clash ran on a partial model at exit 0; now every native entry point
raises `IfcfastError` naming the byte offset, and `ifcfast-bundle`
exits 2), not-a-STEP-file, a typo'd table / entity / mode (GH #71) —
raise **regardless** of `strict`; they're never silenced.

Non-fatal anomalies the native indexer sees (an unknown SI prefix, a
broken conversion chain, a missing `FILE_SCHEMA`, duplicate STEP ids)
are collected on `m.warnings` (a `list[str]`, also in
`m.summary()["warnings"]` and the `index` CLI JSON) — the library never
prints. When `unit_resolved` is `False` the strict message quotes the
indexer's reason.
...
- **Length unit: `unit_scale` / `unit_resolved` / `length_unit`**
  ...
  - **declared-but-unresolved** (broken conversion chain) →
    `unit_resolved == False`; a real unit is masked. An unknown SI
  prefix (`.MILLLI.`) is in this class since GH #149 — it used to fall
  through to metres silently. Under
...
- **All CLI subcommands accept `--json`** and emit a stable JSON shape.
  See `python/ifcfast/cli.py` for the exact keys. A top-level `"path"`
  is present on every subcommand **except `bundle`**, which names its
  output as `"bundle_dir"` instead. `"tables"` is NOT universal — it
  comes from `m.summary()`, so only `index` and `demo` carry it. Read
  the keys you need from the payload; don't assume a shared envelope.
...
typed reason instead of an opaque `Fallback`. Every NATIVE entry point
that runs `cut_openings` (`_core.mesh_qto`, `_core.extract_meshes`,
`_core.write_gltf`) returns a dict carrying them — but on the **Python**
surface only **`m.to_gltf()`** hands that dict back. `m.meshes()`
returns a `MeshList` and `m.mesh_qto()` returns
`(products_df, per_surface_df)`; both drop the counters on the floor.
If you need cut diagnostics for a model, call `m.to_gltf()`. The
counters are:
...
## Federated floors (`ifcfast.federated_floors`)

Discipline models name the same physical level differently (`"Plan 2"`
in ARK, `"E02"` in RIE). This module clusters storeys by elevation
across models and applies a rule to synthesise ONE label per level —
the equivalent of Solibri's `Federated Floor` column.

```python
from ifcfast.federated_floors import (
    StoreyInfo, synthesise_federated_floors,
    default_rule, make_prefix_rule, load_rule_from_yaml,
...
out = synthesise_federated_floors(storeys, rule=make_prefix_rule("C-"),
                                  tolerance_mm=100.0)
# -> {(model_name, storey_name): federated_label}
```

It is deliberately **project-agnostic**: it ships the 1-D elevation
clusterer, the rule protocol, `default_rule` (most-common-name), the
`make_prefix_rule` factory, `drop_leading_zero`, a rule registry
(`register_rule` / `get_rule` / `known_rules`), and a YAML loader.
Project-specific alias tables live with the project, loaded via
`load_rule_from_yaml`. `elevation_mm` is millimetres — convert first if
your file is authored in metres.
...
ifcfast cache   FILE  --json       # inspect the cache FOR THAT FILE
...
#                     for both read and write (cold parse every time)
#   --strict/--no-strict   loud-failure policy (see "Strict mode");
#                     on every subcommand except `cache` and `bundle`
#   --with-data       types — also extract materials + classifications
#                     per type (slower)
#   --samples N       types — sample GUIDs per type (default 3)
#   --top N           types (default 20, pretty mode only) /
...
drifts from the repo root. After editing this file, run
