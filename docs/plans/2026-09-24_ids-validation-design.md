...
| **predefinedType** | Order: occurrence PredefinedType; `USERDEFINED` → ObjectType; missing or `NOTDEFINED` → the type's PredefinedType; the type's `USERDEFINED` → ElementType. Follows `ifcopenshell.util.element.get_predefined_type` [S]. | Generated `PREDEF_POS`, `OBJTYPE_POS`, `ELEMTYPE_POS` per entity per schema; `defines_by_type` edges (lazy) |
...
```python
ifcfast.validate_ids(ids, ifc, *, on_unsupported="raise") -> IdsReport
Model.validate_ids(ids, *, on_unsupported="raise") -> IdsReport
